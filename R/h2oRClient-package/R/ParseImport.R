...
h2o.rm <- function(object) {
  if(class(object) != "H2OParsedData") stop("object must be a H2O parsed data object")
  res = h2o.__remoteSend(object@h2o, h2o.__PAGE_REMOVE, key=object@key)
}
...
h2o.importFolder <- function(object, path, pattern = "", key = "", parse = TRUE, header, sep = "", col.names, version = 1) {
  ...
  h2o.__MYPAGE = ifelse(version == 1, h2o.__PAGE_IMPORTFILES, h2o.__PAGE_IMPORTFILES2)
  
  # if(!file.exists(path)) stop("Directory does not exist!")
  # res = h2o.__remoteSend(object, h2o.__PAGE_IMPORTFILES2, path=path)
  res = h2o.__remoteSend(object, h2o.__MYPAGE, path=path)
...
h2o.uploadFile <- function(object, path, key = "", parse = TRUE, header, sep = "", col.names, silent = TRUE, version = 1) {
  ...
  if(version != 1 && version != 2) stop("version must be either 1 (ValueArray) or 2 (FluidVecs)")
  h2o.__MYPAGE = ifelse(version == 1, "/PostFile.json", "/2/PostFile.json")
  
  url = paste("http://", object@ip, ":", object@port, h2o.__MYPAGE, "?key=", path, sep="")
  ...
  if(parse) parsedData = h2o.parseRaw(data=rawData, key=key, header=header, sep=sep, col.names=col.names, version=version) else rawData
}
...
h2o.parseRaw <- function(data, key = "", header, sep = "", col.names, version = 1) {
  ...
  if(version != 1 && version != 2) stop("version must be either 1 (ValueArray) or 2 (FluidVecs)")
  h2o.__MYPAGE = ifelse(version == 1, h2o.__PAGE_PARSE, h2o.__PAGE_PARSE2)
  ...
  if(missing(header) && missing(col.names))
    res = h2o.__remoteSend(data@h2o, h2o.__MYPAGE, source_key=data@key, destination_key=key, separator=sepAscii)
  else if(missing(header) && !missing(col.names))
    res = h2o.__remoteSend(data@h2o, h2o.__MYPAGE, source_key=data@key, destination_key=key, separator=sepAscii, header=1, header_from_file=col.names@key)
  else if(!missing(header) && missing(col.names))
    res = h2o.__remoteSend(data@h2o, h2o.__MYPAGE, source_key=data@key, destination_key=key, separator=sepAscii, header=as.numeric(header))
  else
    res = h2o.__remoteSend(data@h2o, h2o.__MYPAGE, source_key=data@key, destination_key=key, separator=sepAscii, header=as.numeric(header), header_from_file=col.names@key)
...
h2o.importURL <- function(object, path, pattern = "", key = "", parse = TRUE, header, sep = "", col.names, version = 1) {
  if(class(object) != "H2OClient") stop("object must be of class H2OClient")
  if(!is.character(path)) stop("path must be of class character")
  if(nchar(path) == 0) stop("path must be a non-empty string")
  if(!is.character(key)) stop("key must be of class character")
  ...
  if(version == 1) {
    ...
    h2o.importFile(object, path, key, parse, header, sep, col.names, version)
  }
}
